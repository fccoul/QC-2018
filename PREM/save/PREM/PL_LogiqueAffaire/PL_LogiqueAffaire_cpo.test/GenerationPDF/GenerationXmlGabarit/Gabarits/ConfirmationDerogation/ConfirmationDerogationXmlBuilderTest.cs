...
namespace RAMQ.PRE.PL_LogiqueAffaire_cpo.test.GenerationPDF.GenerationXmlGabarit.Gabarits.ConfirmationDerogation
{
    /// <summary> 
    ///  Essais unitaire sur la classe "ConfirmationDerogationXmlBuilder"
    /// </summary>
    /// <remarks>
    ///  Auteur : Alexis Garon-Michaud <br/>
    ///  Date   : Mars 2017
    /// </remarks>
    public class ConfirmationDerogationXmlBuilderTest : XmlBuilderTestBase
    {
        [Fact]
        public void LorsInstanciation_PasserUnSerialiseurNull_DevraitRetournerArgumentNullException()
        {
            ...
            {
                var builder = new ConfirmationDerogationXmlBuilder(null);
            });
        }
        ...
        public void Construire_PriorietesEstNulle_DevraitLancerUnArgumentNullException()
        {
            Assert.Throws<ArgumentNullException>(() =>
            {
                ...
                var builder = new ConfirmationDerogationXmlBuilder(serialiseurXml);
        ...
        public void Construire_PriorietesContientMoinsPrioriteQueRequis_DevraitLancerUnArgumentOutOfRangeException()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                var serialiseurXml = Substitute.For<ISerialiseurXml>();
                var builder = new ConfirmationDerogationXmlBuilder(serialiseurXml);
                IList<string> priorites = CreerPriorites(ConfirmationDerogationXmlBuilder.NombrePrioriteRequis - 1);
        ...
        public void Construire_Priorites_DevraitMapperCorrectementLesPrioritesDansLeTexteDesColonnes()
        {
            var serialiseurXml = Substitute.For<ISerialiseurXml>();
            var builder = new ConfirmationDerogationXmlBuilder(serialiseurXml);
            IList<string> priorites = CreerPriorites(ConfirmationDerogationXmlBuilder.NombrePrioriteRequis);
            var sortie = builder.Construire(priorites);
